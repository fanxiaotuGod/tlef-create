...
router.post('/:id/regenerate', authenticateToken, validateMongoId, asyncHandler(async (req, res) => {
  ...
  try {
    // Generate new question content using AI-enhanced prompts
    console.log('🔄 Regenerating question with AI prompts...');
    const detailedPrompts = await generateDetailedPrompts(
      question.learningObjective.text,
      question.type,
      1 // Generate just one prompt for regeneration
    );
    
    const newQuestionData = await generateQuestionFromPrompt(
      detailedPrompts[0].detailedPrompt,
      question.type,
      detailedPrompts[0].subObjective
    );
    ...
    question.generationMetadata = {
      ...
      regenerationMethod: 'AI-enhanced-prompting'
    };

    // Add to edit history
    await question.addEdit(userId, 'AI regeneration with enhanced prompts', previousData);
